...
if __name__ == '__main__':
    ...
    target = 24
    result = find_numbers(arr, target)
    print(result)
...
# 3- For the first number, the if statement checks if the difference between the target and the current number is in the dictionary, but it's empty so the code block inside the if statement is not executed.
